    ...
    <div className="flex flex-col h-screen bg-gray-950 text-gray-100 font-sans">
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 py-2 border-b border-gray-800">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Admin panel button — admin only (desktop only; appears in mobile action bar on small screens) */}
          {board.isAdmin && (
            <button
              onClick={() => setShowAdmin(true)}
              title="Admin panel"
              className="hidden md:block text-accent-500 hover:text-accent-400 transition-colors"
              style={{ cursor: "pointer" }}
            >
      ...
      {board.currentUser && !board.isAgent && (
        <div className="flex-shrink-0 flex md:hidden items-center justify-end gap-4 px-6 py-1.5 border-b border-gray-800">
          {board.isAdmin && (
            <button
              onClick={async () => {
                const n = board.columns.length + 1;
